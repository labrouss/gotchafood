...
export const hireStaff = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const schema = z.object({
      ...
      hourlyRate:    z.coerce.number().positive().optional().or(z.literal('')).transform(v => v === '' ? undefined : Number(v)),
...
export const updateStaff = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const schema = z.object({
      ...
      hourlyRate:    z.coerce.number().positive().nullable().optional().or(z.literal('')).transform(v => v === '' ? null : (v ? Number(v) : null)),
      emergencyName: z.string().optional(),
      emergencyPhone:z.string().optional(),
      notes:         z.string().optional(),
    });
