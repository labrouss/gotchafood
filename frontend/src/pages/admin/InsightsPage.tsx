...
import { printContent, generateReportHTML } from '../../utils/print.util';
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="flex justify-between items-start">
        ...
        <button
          onClick={() => {
            const title = `Performance_Report_${days}_Days`;
            const html = generateReportHTML(d, `Insights Summary (${days} Days)`);
            printContent(html, title);
          }}
          className="bg-white/20 hover:bg-white/30 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 transition"
        >
          🖨️ Print Report
        </button>
      </div>
