...
tar cvfz ~labrouss/gotchafood.v${VERSION}.tar.gz --exclude="node_modules" --exclude="backup-*" ./*
echo "--------------------------------------------------------------------------------------------------"
if [ -f ~labrouss/gotchafood.v${VERSION}.tar.gz ] ; then
	echo "OK ! File backup file created succesfully"
        ls -ltr ~labrouss/gotchafood.v${VERSION}.tar.gz 
else
